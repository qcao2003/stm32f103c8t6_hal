...
/* USER CODE BEGIN 0 */
uint8_t temp = 2;//这里定义一个变量

usartRecvType_t usart1Recv = {0};
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		// 当接收的最后一个字符为'\n'作为结束符
		if(usart1Recv.recvData == '\n')
		{
			usart1Recv.recvFlag = 1;  //标志位
			usart1Recv.recvNum = 0;
		}
		else // 接受的数据保存到缓冲区
			usart1Recv.recvBuff[usart1Recv.recvNum++] = usart1Recv.recvData;  
